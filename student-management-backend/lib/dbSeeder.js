...
class DBSeeder {
    ...
    seed() {
        ...
        for (i = 0; i < l; i++) {
            var nameGenderHost = studentNames[i].split(',');
            ...
            for (j = firstOrder; j <= lastOrder && j < n; j++) {
                ...
                student.orders.push(o);
            }
            student.orderCount = student.orders.length;

            student.save((err, cust) => {
                if (err) {
                    console.log(err);
                } else {
                    console.log('inserted student: ' + cust.firstName + ' ' + cust.lastName);
                }
            });
        }
